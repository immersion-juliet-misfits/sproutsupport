...
const MeetupCard = ({
  ...
  const joinMeetup = (): void => {
    axios
      .post('meetup/AttendeeCreate', { userId: user.id, meet_id: card.id })
      .then(() => {
        refresh();
        showSwitch(false, {});
      })
            ...
            {user.id === card.userId && (
              <>
                ...
                <Button
                  ...
                  bg={'#6EB257'}
                  onClick={() => {
                    updateSwap ? setUpdateSwap(false) : setUpdateSwap(true);
                  }}
                >
                  <EditIcon />
                </Button>
