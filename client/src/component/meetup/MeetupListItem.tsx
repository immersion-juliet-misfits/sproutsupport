...
const MeetupListItem = ({
  ...
  const joinMeetup = (): void => {
    axios
      .post('meetup/AttendeeCreate', { userId: user.id, meet_id: group.id })
      .then(() => {
        refresh();
      })
      .catch((err) => {
      ...
      <Card>
        <CardBody bg={'#C5E063'}>
          ...
          <Box bg={'#6EB257'} my={'10px'}>
            ...
            <div>
              {'time left ' +
                (group.timeleft !== undefined ? group.timeleft : 'calculating')}
            </div>
          </Box>
