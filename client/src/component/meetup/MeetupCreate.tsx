...
const MeetupCreate = ({
  ...
  const getweather = (): void => {
    ...
    if (city.length >= 4 && st.length >= 4 && dateTime.length >= 19) {
      const monthCheck =
        parseInt(dateTime?.[0] + dateTime?.[1]) === parseInt(time[0] + time[1]);
      const yearCheck =
        parseInt(
          dateTime?.[6] + dateTime?.[7] + dateTime?.[8] + dateTime?.[9]
        ) === parseInt(time[6] + time[7] + time[8] + time[9]);
      ...
      axios
        ...
        .then(({ data }) => {
          if (monthCheck === true && yearCheck === true) {
            const passCheck =
              parseInt(dateTime?.[3] + dateTime?.[4]) -
              parseInt(time[3] + time[4]);
            if (passCheck < 0) {
      ...
      if (dateTime.length < 19 && city.length >= 4 && st.length >= 4) {
        setWarnMessage(
          'please fill in a date (example: 01/01/2024 1:01 am/pm)'
        );
