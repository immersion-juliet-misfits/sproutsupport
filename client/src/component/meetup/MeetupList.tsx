...
const MeetupList = ({
  ...
  const [warnMessage, setWarnMessage] = useState(
    'please fill in both city and state'
  );
  ...
  const getweather = (): void => {
    if (city.length >= 4 && st.length >= 4) {
      axios
        .get(`/meetup/weather?city=${city}&state=${st}`)
        .then(({ data }) => {
  ...
  useEffect(() => {
    if (
      ...
      if (
        dateTime[17] + dateTime[18] === 'pm' ||
        dateTime[17] + dateTime[18] === 'am'
      ) {
        if (
          location.length > 0 &&
          eventName.length > 0 &&
  ...
  return (
    <>
      {swap === 'none' && (
        <>
          <Box bg={'#6EB257'} position='relative' top='-40px' h={'60px'}>
            <Button
              bg={'#C5E063'}
              isActive={currentState === 'yours' ? false : true}
              onClick={() => {
            ...
            <Button
              bg={'#C5E063'}
              isActive={currentState === 'joined' ? false : true}
              onClick={() => {
            ...
            <Button
              bg={'#C5E063'}
              isActive={currentState === 'public' ? false : true}
              onClick={() => {
                  ...
                  yours.map((group, i) => {
                    ...
                    return (
                      <MeetupListItem
                        ...
                        remove={meetupDelete}
                        swap={meetupSwap}
                        createSwapUpdate={createSwapUpdateCheck}
                        isJoined={false}
                        refresh={refresh}
              ...
              <>
                {join.length === 0 && (
                  <Alert status={'warning'}>
                    <AlertIcon />
                    <AlertDescription>
                      you don't have any meetups you planing to join
                    </AlertDescription>
                  </Alert>
                )}
              </>
              ...
              <>
                {yours.length === 0 && (
                  <Alert status={'warning'}>
                    <AlertIcon />
                    <AlertDescription>
                      you don't have any meetup events
                    </AlertDescription>
                  </Alert>
                )}
              </>
      ...
      {swap === 'update' && (
        <Box position='relative' left='360px' top='-362px'>
          {selecDate}
        </Box>
      )}
    </>
